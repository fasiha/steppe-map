...
def search(lon, lat, x, y, proj, vec2dictfunc, init, order=2):
    ...
    xynew, x2t, t2x, *rest = postfit(xy, np.vstack(pbest(lon, lat)))
    return sols[best], xynew, pbest, x2t, t2x, rest
...
def image_show(x,
    ...
    lonrange = [-180, 180]
    latrange = [-85, 85]
    for l in range(*lonrange, 10):
        plt.plot(*ll2pix2(np.ones(100) * l, np.linspace(*latrange, 100), x2t, outproj), 'r', lw=.5)
    for l in range(*latrange, 10):
        plt.plot(*ll2pix2(np.linspace(*lonrange, 100), np.ones(100) * l, x2t, outproj), 'r', lw=.5)
